...
Lua 5.1 的语法分析器（Parser）负责将词法分析器产生的Token流转换为抽象语法树，并同时生成字节码。本文档深入分析 `lparser.h` 和 `lparser.c` 的实现，详细解释Lua语法分析的机制和代码生成过程。
...
typedef enum {
  ...
  VINDEXED,    // 表索引 (info = 表寄存器, aux = 索引寄存器或常量)
  VJMP,        // 跳转指令 (info = 指令PC)
  VRELOCABLE,  // 可重定位表达式 (info = 指令PC)
  VNONRELOC,   // 非重定位表达式 (info = 结果寄存器)
  VCALL,       // 函数调用 (info = 指令PC)
...
typedef struct expdesc {
  ...
  int t;                  // "true时退出"的跳转链表
  int f;                  // "false时退出"的跳转链表
} expdesc;
```

**设计要点**：
- `t`和`f`字段用于逻辑表达式的短路求值
- `info`和`aux`的含义根据表达式类型而变化
- 支持延迟代码生成和优化
